package com.xcc.beziercurveshoppingcart;
...
import com.xcc.beziercurveshoppingcart.adapter.GoodsAdapter;
...
public class MainActivity extends AppCompatActivity {
    ...
    private void addGoodsToCart(ImageView goodsImg){
        // 创造出执行动画的主题goodsImg（这个图片就是执行动画的图片,从开始位置出发,
        // 经过一个抛物线（贝塞尔曲线）,移动到购物车里）
        ...
        //开始掉落的商品起始点：商品起始点-父布局起始点+该商品图片的一半
...
//        valueAnimator.setInterpolator(new AccelerateInterpolator());
    ...
    private void addData() {
        ...
        goodsModel.setmGoodsBitmap(BitmapFactory.decodeResource(getResources(),R.drawable.goods_one));
        mData.add(goodsModel);

        goodsModel=new GoodsModel();
        goodsModel.setmGoodsBitmap(BitmapFactory.decodeResource(getResources(),R.drawable.goods_two));
        mData.add(goodsModel);
